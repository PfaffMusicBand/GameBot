...
botversion = 1.2 #version du scripte
r= "n"#ne pas modifier
...
async def versions(type, ctx):
    ...
    if type == "commande":
        ...
        embed.add_field(name="Version recommandée :", value=recommanded, inline=False)
        embed.add_field(name="Dernière version :", value=laster, inline=False)
...
async def on_message(message: discord.Message):
    if message.author.id == bot.user.id or message is discord.Interaction:
        ...
        if Botloader.auto_mod.check_word(message.content) == True:
            guild = bot.get_guild(Botloader.Bot.BotGuild)
            ...
            embed.add_field(name="Link", value=f"[Link!]({message.jump_url})")
            if message.attachments:
                for attachment in message.attachments:
